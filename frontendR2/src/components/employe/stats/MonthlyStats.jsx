...
import { useMemo } from 'react'; // <-- Ajoutez cet import en haut
...
const MonthlyStats = ({ data }) => {
 ...
 const chartOptions = useMemo(() => ({
  ...
  // Par :
const statusDistribution = {
  labels: ['Complétées', 'En cours', 'En retard'].filter((_, i) => [data.stats.completed, data.stats.inProgress, data.stats.late][i] > 0),
  datasets: [{
    data: [data.stats.completed, data.stats.inProgress, data.stats.late].filter(val => val > 0),
    backgroundColor: ['#06d6a0', '#ffd166', '#bc313d'].filter((_, i) => [data.stats.completed, data.stats.inProgress, data.stats.late][i] > 0),
    borderWidth: 0
  }]
};
