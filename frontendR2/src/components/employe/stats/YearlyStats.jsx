...
const YearlyStats = ({ data }) => {
  ...
  return (
    <div className="stats-card yearly-stats">
      ...
      // Dans YearlyStats.js, remplacez la section "Analyse des performances" par :
  ...
  <Row gutter={[16, 16]}>
    <Col xs={24} md={12}>
      <div style={{ height: '300px' }}>
        <StatsCharts
          type="bar"
          data={data.monthlyStats.map(month => ({
            ...month,
            month: new Date(month.month).toLocaleString('fr-FR', { month: 'short' }),
            'Tâches complétées': month.completed,
            'Tâches en retard': month.late
          }))}
          xField="month"
          yField={['Tâches complétées', 'Tâches en retard']}
          height={300}
      ...
      <div style={{ height: '300px' }}>
        <StatsCharts
          ...
          data={data.monthlyStats.map(month => ({
            ...month,
            month: new Date(month.month).toLocaleString('fr-FR', { month: 'short' }),
            'Taux de complétion': month.completionRate
          }))}
          xField="month"
          yField={['Taux de complétion']}
          height={300}
          colors={['#1d3557']}
          legend={{
            position: 'top',
