  ...
  try {
    upload(req, res, async (err) => {
      ...
      await processImage(req, res, async () => {
        ...
        io.to(`user_${req.user._id}`).emit("project_created", projectWithUrls);
        // Émettre une notification aux employés assignés
        if (newProject.assignedEmployees.length > 0) {
          const sender = await User.findById(req.user._id).select("name");
          const notificationPromises = newProject.assignedEmployees.map(
            async (employeeId) => {
              const notification = new Notification({
                recipient: employeeId,
                sender: req.user._id, //celui qui a creer le projet
                project: newProject._id,
                message: ` "${sender.name}" vous a été assigné un  nouveau projet "${newProject.name} "`,
                type: "project_created",
              });
              await notification.save();
              io.to(`user_${employeeId}`).emit(
                "project_created",
                projectWithUrls
              );
  ...
  try {
    ...
    project.assignedEmployees =
      project.assignedEmployees?.map((emp) => ({
        ...emp,
        profilePhoto: emp.profilePhoto
          ? `${baseUrl}/public/${emp.profilePhoto}`
          : null,
        profilePhotoThumb: emp.profilePhotoThumb
          ? `${baseUrl}/public/${emp.profilePhotoThumb}`
          : null,
      })) || [];
...
exports.deleteProject = async (req, res) => {
  try {
    ...
    // Émettre l'événement avant la suppression
    const io = req.app.get("io");
    io.to(`user_${req.user._id}`).emit("project_deleted", {
      _id: project._id,
      name: project.name,
    });
    // Créer des notifications avant de supprimer
    if (project.assignedEmployees.length > 0) {
      ...
      const notificationPromises = project.assignedEmployees.map(
        async (employeeId) => {
          const notification = new Notification({
            ...
            sender: req.user._id,
            project: project._id,
            message: `Le projet "${project.name}" a été supprimé par "${sender.name}" `,
            type: "project_deleted",
          });
          await notification.save();

          io.to(`user_${employeeId}`).emit("new_notification", notification);
          io.to(`user_${employeeId}`).emit("project_deleted", {
            _id: project._id,
            name: project.name,
          });
        }
      ...
      await Promise.all(notificationPromises); //attend que les 3 notifications soient souvegardes dans la base pour continuer et Attend que toutes soient résolues (await)
    }
